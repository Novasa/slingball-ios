...
- (id) init {
    if (self = [super init]) {
        ...
        _vertices = malloc((kMaxRibbonSegments * 2) * sizeof(NVColoredVertex));
        
        _startColor = malloc(sizeof(NVColor4f));
        _endColor = malloc(sizeof(NVColor4f));
        
        NVColor4fFill(_startColor, 1, 1, 1, 1);
...
- (void) start {
    ...
    _initialRadius = _body.scaleHead * 0.9f;
}
...
- (void) render {
    ...
    int j = 0;
    for (int i = 0; i < kMaxRibbonSegments * 2; i += 2) {
        SBSlingRibbonSegment* segment = [_controller segmentAtIndex: j];
        
        NVColoredVertex a;
        NVColoredVertex b;
        
        float t = (float)j / kMaxRibbonSegments;
        ...
        NVColor4f color = 
            NVColor4fMake(
                lerp(_startColor->red, _endColor->red, t), 
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, kMaxRibbonSegments * 2);
}
